...
//! Capsules for the SHAKE extensible output function (XOF).
//!
//! Command interface
//! -----------------
//!
...
use kernel::hil::oneshot_digest::{Shake128, Shake256};
use kernel::syscall::{CommandReturn, SyscallDriver};
use kernel::{ErrorCode, ProcessId};

macro_rules! oneshot_shake {
    {
        ...
        impl<D: $hil> $capsule<D>  {
            /// Command handler for `shake` (command #1).
            fn command_shake(
                &self,
                ...
                self.grant.enter(calling_process, |_, kernel_data| {
                    require_ro_buffer(kernel_data, ro_allow::INPUT, None, |input| {
                        require_rw_buffer(kernel_data, rw_allow::DIGEST, None, |mut digest| {
                            <D as $hil>::digest(&input, &mut digest)
                        })
                    })
                })?
            }
        ...
        impl<D: $hil> SyscallDriver for $capsule<D>  {
            fn command(
                ...
                CommandReturn::from(match command_num {
                    command::EXISTS => Ok(()),
                    command::SHAKE => self.command_shake(calling_process),
...
oneshot_shake! {
    capsule = OneshotShake128,
    hil = Shake128,
}
oneshot_shake! {
    capsule = OneshotShake256,
    hil = Shake256,
}
