...
impl kernel::syscall::UserspaceKernelBoundary for SysCall {
    ...
    unsafe fn switch_to_process(
        ...
        100: // _start_app_trap
          ...
          la    s0, 300f            // Load _return_to_kernel into s0.
    ...
    unsafe fn switch_to_process(
        ...
        100: // _start_app_trap
          ...
          // of _return_to_kernel into it, as this will be where we jump on
          // the mret instruction, which leaves the trap handler.
          la    s0, 500f            // Load _return_to_kernel into t0.
          csrrw s0, mepc, s0        // s0 <- mepc[app pc] <- _return_to_kernel
          sw    s0, 31*4(s1)        // Store app's pc in stored state struct.
          ...
          // s2, since s0 and s1 shouldn't be clobbered.
          li   s2, 0b0
          ...
          // trap handler so that it does not fire again.
          //
          // Additionally, we'll use this value to determine if we should switch
          // over to the interurpt stack. Note that we do NOT switch stacks upon
          // receiving a non-maskable interrupt (NMI), as we may want to support
          // recoverable NMIs in the future, and without
          //
          ...
          // Next, check if this was a non-maskable interrupt (NMI).
          // Since NMIs can only be detected by the value in the mcause
          ...
          //
          // For convenience, we pass the value in mcause to
          // _check_mcause_for_nmi as an argument, and expect back a
          // boolean. If we receive back a non-zero return value (i.e. a return
          // value of `true` from an extern C function), we interpret this as an
          // NMI and skip switching stacks for the reason detailed above.
          jal  ra, _check_mcause_for_nmi
          bnez a0, 200f

          // If we've reached this point, this was indeed an interrupt which
          // is *not* an NMI. We set the flag in s2 to indicate this.
          li   s2, 0b1
        ...
        200: // _start_app_trap_call

          // Finally, call the interrupt disable function, with mcause already
          // loaded in a0.
          //
          // This may clobber all caller-saved registers. However, at this
          // stage, we only restored `sp`, `s1`, and the registers above, all
          // of which are saved. Thus we don't have to worry about the function
          // call clobbering these registers.
          //
          jal  ra, _disable_interrupt_trap_rust_from_app

        300: // _start_app_trap_continue

          // Now, on return, we want to recall using the flag in s2 if we had
          // a non-NMI interrupt to determine whether we need to move the stack
          // pointer back from the interrupt stack to the kernel stack. If there
          // wasn't an interrupt, we can skip to restoring registers from the
          // stack.
          beqz s2, 400f
